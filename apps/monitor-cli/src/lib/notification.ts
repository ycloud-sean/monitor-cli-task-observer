  ...
  if (task.hostApp === "cursor") {
    ...
    try {
      const { stdout } = await execFileAsync("osascript", [
        ...
        "end try",
        "-e",
        'set matchesDocument to false',
        "-e",
        'if targetDocument is not "" then set matchesDocument to (documentValue is targetDocument)',
        "-e",
        'set matchesTitle to false',
        "-e",
        'if targetTitle is not "" then set matchesTitle to (titleValue is targetTitle)',
        "-e",
        'set hasTargetGeometry to not ((item 1 of targetPosition is -1) or (item 2 of targetPosition is -1) or (item 1 of targetSize is -1) or (item 2 of targetSize is -1))',
        "-e",
        'set matchesGeometry to hasTargetGeometry and (positionValue is targetPosition) and (sizeValue is targetSize)',
        "-e",
        'return matchesDocument or (matchesTitle and matchesGeometry)',
        "-e",
        "end tell",
        "-e",
        "end tell"
