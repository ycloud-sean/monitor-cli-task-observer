...
```bash
pkill -f 'monitord.js' || true
brew uninstall monitor 2>/dev/null || true
brew untap ycloud-sean/monitor 2>/dev/null || true
rm -f ~/.local/bin/monitor ~/.local/bin/monitord
rm -rf ~/.monitor/monitor-cli-task-observer
rm -rf ~/.monitor-data
rm -rf ~/.cursor/extensions/liangxin.monitor-cursor-bridge-0.1.0
...
- Homebrew 安装：在 Cursor 里执行 `monitor codex` 或 `monitor claude` 时，`monitor` 会自动补装或更新 Cursor bridge
...
- 点击“打开任务”后：
  ...
  - `Cursor` 会优先按工作区名把正确窗口切到前台，再切到对应终端

补充说明：

- 在 macOS 多桌面场景下，`Cursor` 的跨桌面切换依赖工作区名路由，不再只靠 `AXRaise`
- 为了让跨桌面路由稳定命中，目标窗口最好是真实文件夹或 workspace 窗口，不要长期停留在 `无标题 (工作区)`
...
而且在 macOS 多桌面场景下，还有一个额外问题：

- `System Events -> process "Cursor" -> windows` 只能稳定看到当前桌面的窗口
- 目标 Cursor 在另一个桌面时，外部脚本有时只能看到当前窗口，甚至直接拿不到任何窗口

这也是最开始会出现“两个任务都跳到同一个 Cursor 窗口”或者“只能回到当前桌面上的 Cursor”的根因。

### 3. Cursor bridge 怎么解决这个问题

现在的做法是把“窗口路由 + 终端映射”都尽量放到 Cursor 内部完成：

1. `monitor` 启动任务时，向 Cursor 打一个 `cursor://.../register?taskId=...` URI，并带上当前 monitor 进程信息
2. Cursor bridge 扩展收到后，优先用进程父链定位真实终端，再把 `taskId -> terminal processId` 持久化下来
3. 同时记录当前窗口快照，里面至少包含 `workspace`、标题和几何信息
4. 之后用户点击“打开任务”时，daemon 再打开 `cursor://.../focus?taskId=...`
5. 如果当前收到请求的 Cursor 窗口不是目标工作区，bridge 会先调用 Cursor 内建的工作区路由命令，把请求转发到目标工作区窗口
6. 只有当这条内部路由失败时，才回退到外部窗口匹配、应用激活和共享队列兜底
7. 真正归属该任务的 Cursor 窗口收到请求后，会按“内存映射 → 持久化终端 pid → 任务进程父链 → 唯一 cwd”顺序恢复终端，再对对应 terminal 调 `show(false)` 并聚焦终端面板

所以 Cursor 这条链路现在不是“猜 UI”，而是：

- 优先由 Cursor 内部按工作区名路由到正确窗口
- 外部脚本只做兜底窗口激活
- 内部扩展负责切到正确终端，并在扩展重载后继续恢复终端

这也是为什么现在 Cursor 在单桌面和多桌面场景下都能稳定跑通，而不是停留在 best-effort 的窗口激活。
